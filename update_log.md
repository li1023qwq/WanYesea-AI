...
- **AI 文章草稿**：后台「文章 → 所有文章」一键批量生成符合子比主题格式的草稿；支持选择模型、关键词与篇数，后台异步生成；AI 生成文章可显示底部声明（样式对齐子比版权声明）
- **AI 评论回复**：评论过审后自动根据文章内容与上下文生成个性化回复；默认以文章作者身份回复，前台可显示 AI 标识
- **评论回复屏蔽**：自动跳过子比「系统快捷回复」与评论者「我的快捷回复」；支持在设置中自定义屏蔽语句（每行一条）

### 改进

- 评论自动回复采用 WP-Cron + `shutdown` 双保险调度，改善本地/Windows 环境 Cron 不触发导致无回复的问题
- 自动回复失败时在后台评论列表显示「AI 回复失败」及悬停错误详情
